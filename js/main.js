// ── THEME INIT (runs before DOM loads to prevent flash) ──────
(function () {
  try {
...
// ============================================================
//  PickMyAI — main.js (clean single file, no duplicates)
// ============================================================
...
function shareResults() {
  if (navigator.share) {
    navigator.share({ title: 'My AI tool matches from PickMyAI', url: location.href });
  } else {
...
function copyResults() {
  ...
  let text = 'My AI matches from PickMyAI:\n\n';
  tools.forEach((t, i) => {
  ...
  text += 'Find yours free at: ' + window.location.origin;
  navigator.clipboard.writeText(text).then(() => {
    const btn = document.querySelector('.btn-copy');
    if (!btn) return;
    btn.textContent = '✓ Copied!';
    btn.classList.add('copied');
    setTimeout(() => { btn.textContent = '📋 Copy results'; btn.classList.remove('copied'); }, 2500);
  });
}
...
function syncURL(answers) {
  try {
    window.history.replaceState({}, '', '?' + new URLSearchParams(answers).toString());
  } catch (e) {}
}
...
function getURLAnswers() {
  try {
    ...
    const a = {};
    keys.forEach(k => { if (p.get(k)) a[k] = p.get(k); });
...
const FALLBACKS = {
  ...
  video: [
    ...
    { name: 'Runway', tagline: 'Next-gen AI video generation', why: 'Generate and edit video from text prompts — most capable AI video tool.', price: '$15/mo', badge: 'power user', url: 'https://runwayml.com' }
  ],
  marketing: [
  ...
  customer: [
    { name: 'Tidio', tagline: 'Affordable AI chat for SMBs', why: 'Easy-to-set-up live chat and AI bot combo built for small businesses.', price: 'Free / $19/mo', badge: 'best match', url: 'https://tidio.com' },
    { name: 'Intercom Fin', tagline: 'AI customer support agent', why: 'Resolves up to 50% of support tickets automatically with high accuracy.', price: '$39/mo', badge: 'budget pick', url: 'https://intercom.com' },
  ...
  finance: [
    { name: 'Dext', tagline: 'Receipt and expense capture', why: 'Snap receipts and auto-sync to your accounting software in seconds.', price: '$20/mo', badge: 'best match', url: 'https://dext.com' },
    { name: 'Vic.ai', tagline: 'AI accounts payable', why: 'Automates invoice processing and approvals with industry-leading accuracy.', price: 'Custom', badge: 'budget pick', url: 'https://vic.ai' },
...
async function getResults() {
  ...
  tools = tools || FALLBACKS[A.goal] || FALLBACKS.writing;
  renderResults(tools);
}
...
function renderResults(tools) {
  ...
  const goalLabels = {
    writing: 'writing', code: 'coding', images: 'images',
    automation: 'automation', research: 'research', video: 'video',
    marketing: 'marketing', customer: 'customer support',
